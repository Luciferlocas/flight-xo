...
export const getFilteredFlights = (
    ...
    if (filter.departureTime !== "") {
        filtered = filtered.filter(
            (flight) => getTimeCategory(flight.departureTimeAirport) === filter.departureTime
        );
    }

    if (filter.arrivalTime !== "") {
        filtered = filtered.filter(
            (flight) => getTimeCategory(flight.arrivalTimeAirport) === filter.arrivalTime
        );
    }
    ...
    if (filter.sortBy === "price") {
        ...
        filtered.sort((a, b) => {
            const timeA = new Date(a.arrivalTime).getTime() - new Date(a.departureTime).getTime();
            const timeB = new Date(b.arrivalTime).getTime() - new Date(b.departureTime).getTime();
            return timeA - timeB;
        });
